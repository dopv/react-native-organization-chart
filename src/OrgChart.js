...
const getNumberTreeFloor = (node, level = 1) => {
  ...
  return Math.max(
    ...node.nodes.map((child) => getNumberTreeLeave(child, level + 1)),
  );
};

// const depthTree = (node) => {
//   let result = 0;
//   if (node?.nodes) {
//     result += node.nodes.length;
//     node = node.nodes;
//   }
//   return result;
// };
...
export default class OrgChart extends React.Component {
  ...
  renderNode({node, level, index, offsetX}) {
    ...
    // const numberlevel = depthTree(node);
    // console.log('__lv: ', numberlevel);
